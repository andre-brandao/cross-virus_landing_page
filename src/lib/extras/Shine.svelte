<script lang="ts">
	...
	let filterId = 'shine-filter-' + Math.random().toString(36).substr(2, 9);
</script>

<svelte:window on:pointermove={onPointerMove} on:scroll={onScroll} />

<svg class={'fixed inset-0 pointer-events-none'}>
...
<div style:filter="url(#{filterId})" {...$$restProps} class={'inline-block'} bind:this={wrapperEl}>
	<slot />
</div>
